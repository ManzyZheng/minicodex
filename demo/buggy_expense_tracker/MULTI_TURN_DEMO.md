...
启动一次 `minicodex-web` 后，在同一个页面依次发送下面四条 Prompt。不要刷新页面，也不要重启服务；这样可以直接展示同一个 Agent Session 保留了历史对话、已读文件集合和工作区修改状态。
...
预期看点：首次命令审批、`2 failed`、read-before-edit、精确编辑、unified diff、`2 passed` 和 `VERIFIED`。
...
```text
不要再增加功能。检查当前 git diff，运行完整测试和一次 sample.csv 的 CLI 冒烟测试；如果失败就修复。最后按“修复 / 新功能 / 兼容修改 / 验证证据”四项简要总结。
```

预期看点：Agent 使用现有上下文完成回归，最终页面给出完整改动链和验证证据，终端仍同步保留工具输出。

## 两分钟录屏建议

- 0:00–0:15：展示页面顶栏中的本机 Workspace、模型与 IDLE 状态。
- 0:15–0:55：完整展示第 1 轮；审批弹窗、失败测试、Diff、通过测试是核心镜头。
